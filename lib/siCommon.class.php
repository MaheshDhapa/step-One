...
class siCommon extends mySql {
    ...
    public function validateData($aValidation) {
		...
		$nCountRecord = count($aUserInfo);
            ...
            if ($aValidationDetails['validation'] == 'isnumberspecialcharacter') {
                $aValidSpecialCharacters = $aValidationDetails['validSpecialCharacter'];
                $sValidSpecialCharacter='';
                foreach ($aValidSpecialCharacters as $aValidSpecialCharacter) {
                    $sValidSpecialCharacter .= '\\'.$aValidSpecialCharacter;
                }
                if (!preg_match("/^[0-9$sValidSpecialCharacter]*$/", $aValidationDetails['value'])) {
                    $oSession->setError($aValidationDetails['field'], $aValidationDetails['message']);
                    $bIsValid = false;
                }
            }
